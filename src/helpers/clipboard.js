...
class ClipboardManager {
  ...
  showAccessibilityDialog(testError) {
    ...
    if (isStuckPermission) {
      dialogMessage = `🔒 DictationApp needs Accessibility permissions, but it looks like you may have OLD PERMISSIONS from a previous version.

❗ COMMON ISSUE: If you've rebuilt/reinstalled DictationApp, the old permissions may be "stuck" and preventing new ones.

🔧 To fix this:
1. Open System Settings → Privacy & Security → Accessibility
2. Look for ANY old "DictationApp" entries and REMOVE them (click the - button)
3. Also remove any entries that say "Electron" or have unclear names
4. Click the + button and manually add the NEW DictationApp app
5. Make sure the checkbox is enabled
6. Restart DictationApp
...
Would you like to open System Settings now?`;
      ...
      dialogMessage = `🔒 DictationApp needs Accessibility permissions to paste text into other applications.
...
4. Add DictationApp to the list and check the box
5. Restart DictationApp
